...
public class OrderController {
    ...
    @ResponseBody
    public String addOrder(String id, HttpServletRequest request,int type,int status) {
    ...
    @PostMapping("/allOrderInfo")
    @ResponseBody
    public UserOrderData allOrderInfo(int page, int limit, HttpServletRequest request) {
        Page pageObj = new Page();
        pageObj.setPage((page - 1) * limit);
        pageObj.setLimit(limit);

        UserOrderData uod = new UserOrderData();
        List<UserOrder> order = service.findAllOrder2(pageObj);
        uod.setCode(0);
        uod.setCount(service.getOrderCount2());
        uod.setData(order);
        uod.setMsg("200");
        return uod;
    }
    ...
    public String editOrder(int orderId) {
        int n = service.editOrder(orderId,2);
        if (n > 0) {
            return "OK";
        }
        return "FAIL";
